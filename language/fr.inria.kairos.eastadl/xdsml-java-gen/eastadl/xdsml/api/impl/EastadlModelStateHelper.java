...
public class EastadlModelStateHelper implements IK3ModelStateHelper{
	...
	public void restoreModelState(K3ModelState state) {
		for(ElementState elemState : state.getOwnedElementstates()) {
			for(Object o : elemState.getSavedRTDs()) {
				...
				try {
					if (n2v.value != null) {
						Method m = EastadlRTDAccessor.class.getMethod("set"+n2v.name, EObject.class, n2v.value.getClass());
					...
					for(Class<?> c : n2v.value.getClass().getInterfaces()) {
						...
						try {
							m = EastadlRTDAccessor.class.getMethod("set"+n2v.name, EObject.class, n2v.value.getClass().getInterfaces()[0]);
							m.invoke(null, elemState.getModelElement(), n2v.value);
